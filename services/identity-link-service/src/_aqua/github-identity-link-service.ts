...
export interface GithubIdentityLinkServiceDef {
    githubRequest: (req: { did: string; username: string; }, reqPeer: { peerId: string; relayPeerId: string; }, callParams: CallParams<'req' | 'reqPeer'>) => void;
githubVerify: (req: { did: string; username: string; }, reqPeer: { peerId: string; relayPeerId: string; }, callParams: CallParams<'req' | 'reqPeer'>) => void;
}
    ...
    peer.internals.callServiceHandler.use((req, resp, next) => {
        ...
        if (req.fnName === 'githubRequest') {
            const callParams = {
                ...req.particleContext,
                tetraplets: {
                    req: req.tetraplets[0],reqPeer: req.tetraplets[1]
                },
            };
            resp.retCode = ResultCodes.success;
            service.githubRequest(req.args[0], req.args[1], callParams); resp.result = {}
        }
...
if (req.fnName === 'githubVerify') {
            ...
            resp.retCode = ResultCodes.success;
            service.githubVerify(req.args[0], req.args[1], callParams); resp.result = {}
        }
