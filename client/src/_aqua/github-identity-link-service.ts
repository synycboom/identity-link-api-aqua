...
export interface GithubIdentityLinkServiceDef {
    githubRequest: (req: { did: string; username: string; }, requestId: string, reqPeer: { hasRelayPeer: boolean; peerId: string; relayPeerId: string; }, callParams: CallParams<'req' | 'requestId' | 'reqPeer'>) => void;
githubVerify: (req: { jws: string; }, requestId: string, reqPeer: { hasRelayPeer: boolean; peerId: string; relayPeerId: string; }, callParams: CallParams<'req' | 'requestId' | 'reqPeer'>) => void;
}
...
export function registerGithubIdentityLinkService(...args: any) {
    ...
    const incorrectServiceDefinitions = missingFields(service, ['githubRequest', 'githubVerify']);
    if (!!incorrectServiceDefinitions.length) {
        throw new Error("Error registering service GithubIdentityLinkService: missing functions: " + incorrectServiceDefinitions.map((d) => "'" + d + "'").join(", "))
    }
    ...
    peer.internals.callServiceHandler.use((req, resp, next) => {
        ...
        if (req.fnName === 'githubRequest') {
            ...
            service.githubRequest(req.args[0], req.args[1], req.args[2], callParams); resp.result = {}
        }
...
if (req.fnName === 'githubVerify') {
            ...
            resp.retCode = ResultCodes.success;
            service.githubVerify(req.args[0], req.args[1], req.args[2], callParams); resp.result = {}
        }
    ...
    const promise = new Promise<void>((resolve, reject) => {
        const r = new RequestFlowBuilder()
                ...
                .configHandler((h) => {
                    ...
                    h.on('getDataSrv', 'req', () => {return req;});
                    h.on('getDataSrv', 'requestId', () => {return requestId;});
                    h.on('getDataSrv', 'reqPeer', () => {return reqPeer;});
